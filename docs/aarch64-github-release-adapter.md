...
| --- | --- | --- | ---: | ---: |
| edge | `aarch64-edge` | `releases/download/aarch64-edge` | 122 bases | 122 bases |
| rc | `aarch64-rc` | `releases/download/aarch64-rc` | 120 bases | metadata-derived fast ring, plus 2 pinned bases only from the `rc` branch |
| stable | `aarch64-stable` | `releases/download/aarch64-stable` | 120 bases | metadata-derived fast ring |
...
2. With no final database available for version comparison, the regular
   builder selects every one of the 122 scoped package bases. Dependencies built
...
The adapter configures 122 package bases from two deliberately separate manifests:

- `config/aarch64-packages` is the reviewed 119-base range that the shared
...
The test suite requires the upstream-aligned manifest to contain 119 unique
...
edge, rc, and stable repository scopes to remain 122/120/120, requires the edge
...
- T3 Code 0.0.38, inherited from the current shared upstream baseline while
