...
[[ ${#upstream_packages[@]} -eq 119 ]] || {
  echo "Expected the reviewed 119-base upstream AArch64 edge scope; found ${#upstream_packages[@]}" >&2
  exit 1
}
[[ ${#packages[@]} -eq 123 ]] || {
  echo "Expected 119 upstream AArch64 bases plus 4 fork additions; found ${#packages[@]}" >&2
  exit 1
}
...
assert_scope_count scope edge 123
assert_scope_count scope rc 121
assert_scope_count scope stable 121
assert_scope_count build-scope edge 123

# A staged release must retain the review boundary between the upstream-aligned
# scope and the fork-only overlay. A combined 123-entry file produces the same
# repository but loses the evidence that only four package bases are downstream.
...
[[ ${#staged_upstream_scope[@]} -eq ${#upstream_packages[@]} ]] || {
  echo "Staged upstream scope lost its 119-base review boundary" >&2
  exit 1
}
...
[[ ${#fork_packages[@]} -eq 4 ]] || {
  echo "Expected exactly 4 explicit fork package bases; found ${#fork_packages[@]}" >&2
  exit 1
}
...
printf '%s\n' "${fork_packages[@]}" | sort -u | diff -u \
  <(printf '%s\n' omarchy-aarch64-keyring omarchy-aarch64-config omarchy-spice-guest-tools hyprland | sort -u) - || {
...
# upstream. The rolling adapter builds the same edge range as upstream; its four
