...
[[ ${#packages[@]} -eq 122 ]] || {
  echo "Expected 119 upstream AArch64 bases plus 3 fork additions; found ${#packages[@]}" >&2
  exit 1
}
...
assert_scope_count scope edge 122
assert_scope_count scope rc 120
assert_scope_count scope stable 120
assert_scope_count build-scope edge 122

# A staged release must retain the review boundary between the upstream-aligned
# scope and the fork-only overlay. A combined 122-entry file produces the same
